...
REPLACEMENT_CHAR = "〇"
# Include the replacement char in the kanji replacement regex so that it too gets cleaned off
# also the kanji repeater character 々, it's included in the japanese punctuation section instead of the
# kanji
KANJI_RE = re.compile(rf"([{REPLACEMENT_CHAR}々\u4e00-\u9faf\u3400-\u4dbf]+)")
print(KANJI_RE)
...
def find_all_indexes(input_str, substring):
    start = 0
    while True:
        start = input_str.find(substring, start)
        if start == -1: return
        yield start
...
def on_kana_highlight_filter(
    ...
    while kanji_pos := text.find(kanji_to_highlight):
        if kanji_pos == -1:
            break
        debug_text += f"\nkanji_pos: {kanji_pos}, text: {text}"
        # print(f"kanji_pos: {kanji_pos} in text: {text}")
        ...
        furigana_end_pos = -1
        debug_text += f"\nfurigana loop range: {range(kanji_pos + len(kanji_to_highlight), len(text))}"
        for i in range(kanji_pos + len(kanji_to_highlight), len(text)):
            if text[i] == "[":
                ...
                furigana += text[i]
        debug_text += f"\nfurigana_pos: {furigana_start_pos} to {furigana_end_pos}, furigana: {furigana}"
        ...
        new_furigana = process_readings(furigana, debug_text)
        debug_text += f"\nnew_furigana: {new_furigana}"
        if new_furigana is None:
            ...
            # Replace this kanji match with a placeholder so we don't match it again
            text = text.replace(kanji_to_highlight, REPLACEMENT_CHAR, 1)
    ...
    # QMessageBox.warning(None,"debug",debug_text + f"final text: {kana_filter(text)}")
    print(debug_text)
