...
class BasicEditorFormLayout(QFormLayout):
    ...
    def __init__(
        ...
        self.note_type_target_cbox.addItems(
            [f'"{model_name}"' for model_name in model_names_list]
        )
        ...
        self.addRow(
            "",
            QLabel(
                """<small>Cards belong to decks, not notes.<br>
    ...
    def update_direction_labels(self, direction):
        ...
        self.target_note_type_label.setText(
            f"<h3>Trigger ({target_type}) note type</h3>"
        )
        self.deck_limit_label.setText(f"<h4>Trigger ({target_type}) deck limit</h4>")

    def init_fields(self, copy_definition: CopyDefinition):
        if copy_definition:
            with suppress(KeyError):
                self.note_type_target_cbox.setCurrentText(
                    copy_definition["copy_into_note_types"]
                )
            ...
            with suppress(KeyError):
                self.copy_on_review_checkbox.setChecked(
                    copy_definition["copy_on_review"]
                )
            with suppress(KeyError):
                self.decks_limit_multibox.setCurrentText(
                    copy_definition["only_copy_into_decks"]
                )
            with suppress(KeyError):
        ...
        if len(models) > 1:
            ...
            for model in models:
                if len(model["tmpls"]) > 1:
                    models_first_templates.append(
                        (model["name"], model["tmpls"][0]["name"])
                    )
    ...
    def set_current_decks(self):
        ...
        if self.decks_init_done:
            current_deck_names = [
                d.strip('""') for d in self.decks_limit_multibox.currentData()
            ]
        else:
            current_deck_names = (
                self.copy_definition.get("only_copy_into_decks", "")
                .strip('""')
                .split('", "')
            )
        ...
        if len(self.selected_models) == 0:
            self.decks_limit_multibox.setPlaceholderText(
            ...
            self.decks_limit_multibox.setPlaceholderText(
                "No decks found for selected note types"
            )
            self.decks_limit_multibox.setDisabled(True)
...
class AcrossNotesCopyEditor(QWidget):
    def __init__(self, parent, copy_definition: CopyDefinition):
        ...
        self.destination_to_source_radio = QRadioButton(
            DIRECTION_DESTINATION_TO_SOURCES
        )
        self.direction_radio_buttons.addWidget(self.destination_to_source_radio)
        self.source_to_destination_radio = QRadioButton(
            DIRECTION_SOURCE_TO_DESTINATIONS
        )
        ...
        self.basic_editor_form_layout = BasicEditorFormLayout(
            ...
            copy_definition,
            extra_top_widgets=[
                (QLabel("<h3>Copy direction</h3>"), self.direction_radio_buttons)
            ],
        )
        ...
        self.form.addRow(
            "<h4>How to select a card to copy from</h4>", self.card_select_hbox
        )
        ...
        self.form.addRow(
            "<h5>Select multiple cards? (set 0 for all)</h5>", card_select_count_hbox
        )
        ...
        self.form.addRow(
            "<h5>Separator for multiple values</h5>", self.card_select_separator
        )
        spacer = QSpacerItem(100, 40, QSizePolicyExpanding, QSizePolicyMinimum)
        self.form.addItem(spacer)

        # Add field-to-field editor
        self.fields_vbox = FieldToFieldEditorVBox(
            self, copy_definition, COPY_MODE_ACROSS_NOTES
        )
        ...
        if copy_definition:
            with suppress(KeyError):
                self.card_query_text_layout.set_text(
                    copy_definition["copy_from_cards_query"]
                )
            with suppress(KeyError):
            ...
            with suppress(KeyError):
                self.card_select_separator.setText(
                    copy_definition["select_card_separator"]
                )
        # Always init the direction, as this will set the checkboxes state, and various
        # labels everywhere
        try:
            direction = copy_definition["across_mode_direction"]
            if direction == DIRECTION_SOURCE_TO_DESTINATIONS:
    ...
    def update_direction_labels(self, direction: DirectionType):
        ...
        with block_signals(
            self.source_to_destination_radio, self.destination_to_source_radio
        ):
            if direction == DIRECTION_SOURCE_TO_DESTINATIONS:
                self.source_to_destination_radio.setChecked(True)
                self.destination_to_source_radio.setChecked(False)
                self.card_query_text_label.setText(
                    "<h3>Search query to get source cards</h3>"
                )
                ...
                self.card_query_text_label.setText(
                    "<h3>Search query to get destination cards</h3>"
                )
    ...
    def update_fields_by_target_note_type(self, models: list[NotetypeDict]):
        ...
        self.variables_vbox.field_to_variable_editor.set_selected_copy_into_models(
            models
        )
...
class WithinNoteCopyEditor(QWidget):
    def __init__(
        ...
        # Add field-to-field editor
        self.fields_vbox = FieldToFieldEditorVBox(
            self, copy_definition, COPY_MODE_WITHIN_NOTE
        )
        self.main_layout.addLayout(self.fields_vbox)
...
class EditCopyDefinitionDialog(ScrollableQDialog):
    ...
    def __init__(self, parent, copy_definition: CopyDefinition = None):
        ...
        self.copy_definition: CopyDefinition = copy_definition
        ...
        self.top_form.addRow(
            "<h2>Name for this copy definition</h2>", self.definition_name
        )
        ...
        self.editor_type_tabs.addTab(
            self.across_notes_editor_tab, COPY_MODE_ACROSS_NOTES
        )
        ...
        set_size_policy_for_all_widgets(
            self.tabs_vbox, QSizePolicyPreferred, QSizePolicyFixed
        )
        ...
        self.resize(
            max(self.sizeHint().width(), int(screen.width() * 0.80)),
            min(self.sizeHint().height() * 1.5, int(screen.height())),
        )
    ...
    def check_fields(self) -> bool:
        ...
        if show_error:
            showInfo(
                f"""Some required required fields are missing:
            ...
            if name_match_count > 1:
                showInfo(
                    "There is another copy definition with the same name. Please choose a unique name."
                )
            return self.accept()
    ...
    def update_fields_by_target_note_type(
        self, basic_editor_layout: BasicEditorFormLayout
    ):
        """
    ...
    def get_copy_definition(self) -> Union[CopyDefinition, None]:
        if self.selected_editor_type == COPY_MODE_ACROSS_NOTES:
            ...
            field_to_field_editor = (
                self.across_notes_editor_tab.get_field_to_field_editor()
            )
            field_to_variable_editor = (
                self.across_notes_editor_tab.get_field_to_variable_editor()
            )
            copy_definition: CopyDefinition = {
                "definition_name": self.definition_name.text(),
